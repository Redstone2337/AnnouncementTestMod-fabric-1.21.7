package net.redstone233.atm.button;
    ...
    public void updateTextLines() {
        ...
        try {
            ...
            if (AnnouncementTestModClient.DEBUG_MODE) {
                AnnouncementTestModClient.LOGGER.info("处理了 {} 行文本，总高度: {}", textLines.size(), totalHeight);
                for (int i = 0; i < textLines.size(); i++) {
                    AnnouncementTestModClient.LOGGER.info("行 {}: {}", i, textLines.get(i).getString());
                }
            }

            totalHeight = textLines.size() * textRenderer.fontHeight;
        } catch (Exception e) {
            AnnouncementTestModClient.LOGGER.error("文本处理失败", e);
