...
//        }
    ...
    protected void init() {
        ...
        if (config.showIcon && config.iconPath != null && !config.iconPath.isEmpty()) {
            try {
                ...
                if (client != null && client.getResourceManager().getResource(iconTexture).isEmpty()) {
                    AnnouncementTestModClient.LOGGER.warn("图标纹理不存在: {}", config.iconPath);
                    iconTexture = null;
                }
            } catch (Exception e) {
                AnnouncementTestModClient.LOGGER.warn("无法加载图标纹理: {}", config.iconPath, e);
                iconTexture = null;
            }
        }
        ...
        if (config.useCustomAnnouncementBackground && config.announcementBackgroundPath != null && !config.announcementBackgroundPath.isEmpty()) {
            try {
                ...
                if (client != null && client.getResourceManager().getResource(backgroundTexture).isEmpty()) {
                    AnnouncementTestModClient.LOGGER.warn("公告背景纹理不存在: {}", config.announcementBackgroundPath);
                    backgroundTexture = null;
                }
            } catch (Exception e) {
                AnnouncementTestModClient.LOGGER.warn("无法加载公告背景纹理: {}", config.announcementBackgroundPath, e);
                backgroundTexture = null;
            }
        }
        ...
        // 调试日志
        if (AnnouncementTestModClient.DEBUG_MODE) {
            AnnouncementTestModClient.LOGGER.info("公告内容: {}", contentText.getString());
            AnnouncementTestModClient.LOGGER.info("内容颜色: 0x{}", Integer.toHexString(contentColor));
        }
            ...
            while (m.find()) {
                ...
                if (segment.startsWith("&#")) {
                    ...
                    try {
                        ...
                        AnnouncementTestModClient.LOGGER.warn("无效的RGB颜色代码: {}", segment);
                    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (config.showIcon && iconTexture != null) {
            ...
            try {
                ...
                AnnouncementTestModClient.LOGGER.warn("无法绘制图标", e);
            }
        }
        ...
        if (AnnouncementTestModClient.DEBUG_MODE && scrollableText != null) {
            context.fill(
    ...
    private boolean isVersionCompatible() {
        try {
            ...
            if (minecraftContainer.isEmpty()) {
                AnnouncementTestModClient.LOGGER.warn("无法找到 Minecraft mod 容器");
                return true;
            }

            String version = minecraftContainer.get().getMetadata().getVersion().getFriendlyString();
            AnnouncementTestModClient.LOGGER.info("检测到 Minecraft 版本: {}", version);

            return isVersionAtLeast(version, 1, 21, 6);
        } catch (Exception e) {
            AnnouncementTestModClient.LOGGER.warn("无法确定 Minecraft 版本", e);
            return true;
        }
    }
    ...
    private boolean isVersionAtLeast(String versionString, int minMajor, int minMinor, int minPatch) {
        try {
            ...
            AnnouncementTestModClient.LOGGER.warn("无法解析版本号: {}", versionString, e);
            return false;
        }
    }
    ...
    private Formatting getFormattingFromColor(int color) {
        ...
        // 如果没有找到匹配的颜色，返回默认值
        AnnouncementTestModClient.LOGGER.warn("未找到匹配的Formatting枚举，颜色: 0x{}", Integer.toHexString(color));
